...
static struct testMessage receiver;
...
void appMain()
{
  ...
  while(1) {
    ...
    if (state == unlocked) {
      ...
      if (appchannelReceivePacket(&receiver, sizeof(receiver), 0)) {
        if(receiver.message == zero){
            state = idle;
        }
        DEBUG_PRINT("X\n");
      }
      ...
      if (appchannelReceivePacket(&receiver, sizeof(receiver), 0) && state == idle && up > 0.001f) {
        DEBUG_PRINT("Waiting for hand to be removed!\n");
        state = lowUnlock;
      }

      if (state == lowUnlock && receiver.message == one && positioningInit && multirangerInit) {
        DEBUG_PRINT("Unlocked!\n");
        state = unlocked;
      }
    
      if (state == lowUnlock && receiver.message != one) {
        state = idle;
      }
